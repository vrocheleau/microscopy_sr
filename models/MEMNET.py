...
class MemNet(nn.Module):
    def __init__(self, in_channels, scale_factor, channels=64, num_memblock=6, num_resblock=6):
    ...
    def forward(self, x):
        x = F.interpolate(x, scale_factor=self.scale_factor, align_corners=True, mode='bicubic')
        residual = x
...
if __name__ == '__main__':
    ...
    model = MemNet(in_channels=1, scale_factor=4)
    out = model(x)
    print(out.shape)